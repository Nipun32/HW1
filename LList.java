...
class LList<T> implements ListInterface<T> {
    ...
    */
    ...
    public void reverse() {
 //here we will take 3 pointer namely previous,present and next which will i first initialize to NULL,firstnode and next of firstnode.
    	//then i will make a loop in which i store previous pointer value in present next and befire this assign next address with present next referenece
    	//until previous reach to last node.
    	//then firstnode set by previous address value.
        // CODE TO BE COMPLETED
    	 //to check if list has more than one element.
    	if(getLength()>1){
        	Node previous,present,present_next;
        	previous=null;
        	present=firstNode;
        	while(present!=null)
        	{
        		present_next=present.getNextNode();
        		present.setNextNode(previous);
        		previous=present;
        		present=present_next;
        	}
        	//present.setNextNode(node1);
        	firstNode=previous;
    	}  	  }
    ...
    public void cycle() {
        ...
        // CODE TO BE COMPLETED  
    	//in linked list we know that the last pointer is null and thus at last next we will store first node address 
    	//and head will point to previous firstnode next which we will store in a nextnode pointer.
    	//here making cycle is much easier then array implementation with time complexity o(n).
